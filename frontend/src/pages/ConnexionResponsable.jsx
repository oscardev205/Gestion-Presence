...
function ConnexionResponsable() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const reponse = await apiResponsable.post('/responsables/connexion', { email, motDePasse });
      localStorage.setItem('tokenResponsable', reponse.data.token);
      localStorage.setItem('responsableInfo', JSON.stringify(reponse.data.responsable));
    ...
    <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
      <div className="w-full max-w-sm anim-apparition">
        <div className="bg-gradient-to-br from-primary-900 via-primary-800 to-primary-400 rounded-t-3xl px-8 pt-10 pb-16 text-center">
          ...
          <h1 className="text-white text-2xl font-semibold tracking-tight">Espace responsable</h1>
          <p className="text-primary-50 text-sm mt-1">Pointage des présences</p>
        </div>
